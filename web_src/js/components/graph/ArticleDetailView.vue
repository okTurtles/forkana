...
/* ArticleDetailView.vue
   The state the graph enters when a bubble is CLICKED: that one article, alone,
   drawn as a 425px circle centred in the canvas, with its excerpt and its
   actions inside it and a Back control that returns to the graph.

   It is entered from the 202px hovered bubble (see FishboneGraph's HOVER/OPEN
   section) and flies out of it: hover grows a bubble in place and shows what
   the article IS, click opens it and shows what you can DO with it.
   ...
   The graph itself is not merely covered while this is open: FishboneGraph
   stops rendering the bubbles and connectors entirely (it re-renders them the
   moment the close starts, so the circle shrinks back INTO its bubble). */
...
/* ──────────────────────────────────────────────────────────────────────────────
   ...
   The timings themselves live in the stylesheet below (300ms travel — the same
   duration and the same ease-in-out cubic as the hover reflow tween in
   FishboneGraph, so the two moves read as one system — 120ms content fade-out
   leading an 80ms shrink delay); the only numbers JS needs are when the element
   may be unmounted: the CSS total plus a frame of slack. */
const CLOSE_TOTAL_MS = 430;   // 120 lead + 80 delay + 300 travel, rounded up
...
onMounted(() => {
  /* Keyboard: the bubble that had focus is gone (the graph stops rendering
  ...
  el.style.transition = '';          // back to 300ms from the stylesheet
...
.detail-layer {
  ...
  --detail-zoom-ms: 300ms;
  /* ease-in-out cubic — the CSS spelling of the easing the hover reflow tween
     uses in JS (easeInOut), so opening this view continues that motion rather
     than starting a different one. */
  --detail-zoom-ease: cubic-bezier(0.65, 0, 0.35, 1);
}
...
.detail-bubble {
  /* Never wider than the viewport allows — on a phone the design's 425px
     circle would be clipped on both sides. */
  width: min(var(--detail-size, 425px), 84vw);
  height: min(var(--detail-size, 425px), 84vw);
  flex: none;
  display: flex;
  align-items: center;
  justify-content: center;
...
/* Content follows the circle rather than travelling with it: it appears once
   the circle is under way (140ms into a 300ms move) and leaves before the
   circle starts back. */
.detail-content,
.detail-back {
  opacity: 0;
...
.detail-content {
  /* Vertical rhythm from the design, on the 8px grid at the design size
     (425px circle): count → 16 → excerpt → 24 → "Read full article" → 16 →
     "View history" → 24 → "Last updated". Signed off as it stands; the two
     steps are expressed as a fraction of the circle so a circle that had to
     shrink (narrow viewport, short canvas) keeps the same proportions and the
     stack still fits inside the arc: 0.0376 × 425 = 16, 0.0565 × 425 = 24. */
  --detail-gap-tight: clamp(8px, calc(var(--detail-size, 425px) * 0.0376), 16px);
  --detail-gap-loose: clamp(12px, calc(var(--detail-size, 425px) * 0.0565), 24px);
